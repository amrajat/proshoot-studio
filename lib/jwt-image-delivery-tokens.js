 ...
 */
export const generateImageDeliveryToken = async (imageKey, jwtSecret) => {
  ...
  try {
    ...
    throw new Error(
      `Failed to generate image delivery token: ${error.message}`
    );
  }
};
...
export const generateSecureImageUrls = async (
  ...
  try {
    ...
    throw new Error(`Failed to generate secure image URLs: ${error.message}`);
  }
};
