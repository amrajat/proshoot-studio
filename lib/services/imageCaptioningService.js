...
async function compressImageForAPI(imageBlob) {
  try {
    ...
    console.log("Compressed file type:", compressedFile);
    // i want to the compressed file in my browser console as image like real image
    console.log("Compressed file:", URL.createObjectURL(compressedFile), "MB");
      ...
      try {
        ...
        if (data.data && data.type === "ic-data") {
          try {
            ...
            console.error("IC data decode error:", decodeError);
            return DEFAULT_CAPTION;
          }
        }
        ...
        clearTimeout(timeoutId);
        console.error("IC fetch error:", fetchError);
...
export async function processImagesWithCaptions(
  ...
  try {
    ...
    // Log the total package size for debugging
    console.log(
      `Package size: ${(packagedData.size / (1024 * 1024)).toFixed(2)}MB`
    );
    ...
    console.error("Data packaging error:", error);
