...
function generateAuthToken() {
  // This should match the server's PROCESSING_SECRET_KEY environment variable
  // In production, this key should be properly secured
  const secretKey =
    process.env.NEXT_PUBLIC_PROCESSING_SECRET_KEY ||
  ...
  // Generate a token using the timestamp and secret - match server implementation exactly
...
function safeBase64Decode(base64Data) {
  if (!base64Data) {
    console.error("Empty base64 data received");
    return "A photograph of JSSPRT.";
  }
  ...
  try {
    ...
    console.error("Standard base64 decoding error:", error);

    try {
      // Second try: Handle URL-safe base64 encoding
      const fixedBase64 = base64Data.replace(/-/g, "+").replace(/_/g, "/");
      return atob(fixedBase64);
    } catch (error2) {
      console.error("URL-safe base64 decoding error:", error2);

      try {
        // Third try: Use TextDecoder for UTF-8 handling
        ...
        console.error("TextDecoder decoding error:", error3);
...
// Add this function to verify the decoded caption is valid
function validateDecodedCaption(caption) {
  // Check if the caption is a valid string and starts with expected prefix
  if (typeof caption !== "string" || caption.length < 10) {
    return false;
  }
  ...
  return nonPrintableRatio <= 0.15;
}

// Add this function to sanitize captions before writing to ZIP
function sanitizeCaptionForZip(caption) {
    ...
    try {
      ...
      try {
        ...
        if (response.status === 401) {
          console.error(
            "Authentication failed for IC generation. Check environment variables."
          );
          return DEFAULT_CAPTION;
        }
        ...
        if (process.env.NODE_ENV === "development") {
          console.log("IC response type:", data.type);
          console.log("IC response status:", data.status);
          if (data.data) {
            console.log("IC response data length:", data.data.length);
          }
        }

        // Handle the obfuscated response
        if (data.data && data.type === "ic-data") {
          // Use the safe decoding function
          const decodedData = safeBase64Decode(data.data);

          // Log the first 50 characters of the decoded data in development
          if (process.env.NODE_ENV === "development") {
            console.log(
              "Decoded caption preview:",
              decodedData ? decodedData.substring(0, 50) + "..." : "null"
            );
          }

          // Validate the decoded caption
          if (decodedData && validateDecodedCaption(decodedData)) {
            return decodedData;
          } else {
            console.error("Received invalid caption data");
            return DEFAULT_CAPTION;
          }
        }
                  ...
                  const processingPromise = new Promise(
                    async (resolve, reject) => {
                      ...
                      try {
                        ...
                        // Sanitize the caption before adding to ZIP
                        const sanitizedCaption = sanitizeCaptionForZip(caption);
                        ...
                        zip.file(metaFileName, sanitizedCaption);
