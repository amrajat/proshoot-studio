...
export async function getCurrentSession() {
  try {
    ...
    if (error) {
      // Check if this is a refresh token error (expected for unauthenticated users)
...
export async function signInWithOAuth(provider) {
  try {
    ...
    if (provider === "google") {
      ...
      // LinkedIn OIDC specific options
    ...
    // Log the provider and options for debugging
    ...
    if (error) {
      ...
      // Don't report this to Sentry as it's likely a user-facing error
      throw new Error(error.message);
    }

    if (!data || !data.url) {
      console.error(`No redirect URL returned for ${provider} OAuth`);
      throw new Error(
        `Failed to get redirect URL for ${provider} authentication`
      );
    }
    ...
    console.log(`Redirecting to OAuth provider URL for ${provider}`);

    // Use Next.js redirect function - this will throw a NEXT_REDIRECT error
    // which is expected and will be caught by the client component
    redirect(data.url);

    // This line will never be reached due to the redirect
    // but we include it for TypeScript/linting purposes
    return { success: true };
  } catch (error) {
    // If this is a NEXT_REDIRECT error, we don't need to log it to Sentry
    // as it's expected behavior
    if (error.message !== "NEXT_REDIRECT") {
      console.error(
        `Unexpected error in signInWithOAuth with ${provider}:`,
        error
      );
      Sentry.captureException(error);
    }
...
export async function createCheckoutLS(
  ...
  try {
    ...
    const newCheckout = {
      productOptions: {
        name: "Proshoot",
        description: "Hyper-realistic professional AI Headshots.",
