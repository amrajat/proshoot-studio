-- Recreate Function again to fix column name casing issue
...
BEGIN
    ...
    END IF;
     ...
     -- Check if user is already a member (Same as before)
    ...
    IF invite_record.transfer_credit_type != 'none' THEN
        -- *** FIX: Use lowercase column names without quotes ***
        transfer_column := CASE invite_record.transfer_credit_type
            WHEN 'starter' THEN 'starter' -- Corrected
            WHEN 'pro' THEN 'pro'         -- Corrected
            WHEN 'elite' THEN 'elite'       -- Corrected
            WHEN 'studio' THEN 'studio'      -- Corrected
            WHEN 'balance' THEN 'balance'     -- Was correct
            ELSE NULL
        END;
        ...
        -- Decrement organization credits (Same EXECUTE format)
        ...
        -- Increment user credits (Same EXECUTE format)
